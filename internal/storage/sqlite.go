...
const (
	sqlGetCases   = "SELECT id, image_path, preview_path, name, type, uploaded FROM photo"
	sqlAddCase    = "INSERT INTO photo (image_path, preview_path, name, type, uploaded) VALUES (?, ?, ?, ?, ?)"
	sqlDelCase    = "DELETE FROM photo WHERE id = ?;"
	sqlUpdateCase = "SELECT id, image_path, preview_path, name, type, uploaded FROM photo"
)
